...
var start_from_block_list = false;
var msg_id = 0;
var history_pages = {};
var last_hash = null;
...
ws.onmessage = function (ev) {
    ...
    if(obj.msg_cmd == EXPLORER_MAIN_PAGE) {
        ...
        for (var i = 0; i < obj.block_list.length; ++i) {
            ...
            var dt = new Date(block.utc * 1000);
        ...
        if(start_from_block_list) {
            block_list_page = $("#top_div_2").clone();
            start_from_block_list = false;
        }
        $("#top_div_2").remove();
        in_main_page = false;
        ...
        block_details += '' +
            ...
            '<tr class="twofield">' +
            '<td>Miner name</td>' +
            '<td><a href="javascript:void(0);" pubkey='  + obj.miner_pubkey + '>' + Base64.decode(obj.miner_name) + '</a></td>' +
            '</tr>' +
            '<tr class="twofield">' +
            '<td>Block reward</td>' +
